...
serve(async (req) => {
  ...
  try {
    ...
    if (override && !override.includes("-beauty-books-world")) {
      override = override.replace("webhook-wsender", "webhook-wsender-beauty-books-world");
    }
    const webhookUrl = override || `${supabaseUrl}/functions/v1/webhook-wsender-beauty-books-world`;
