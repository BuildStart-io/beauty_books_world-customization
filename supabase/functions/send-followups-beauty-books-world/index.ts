...
Deno.serve(async (req) => {
  ...
  const supabase = createClient(supabaseUrl, serviceKey, { db: { schema: 'beauty_books_world_customization' } });
  ...
  try {
    ...
    for (const row of settingsRows || []) {
      ...
      for (const [key, entry] of perContact) {
        ...
        try {
          const res = await fetch(`${supabaseUrl}/functions/v1/send-whatsapp-beauty-books-world`, {
            method: "POST",
            headers: {
